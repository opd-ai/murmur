...
- [ ] **Key material not zeroed before GC** — pkg/identity/keys/keystore.go:150–180 — After decrypting private keys from the Argon2id-encrypted keystore, the plaintext key bytes are copied but the backing array is not zeroed before going out of scope. Per SECURITY_PRIVACY.md §2.1, "Key material must be zeroed before backing arrays become GC-eligible." If an attacker gains memory access during or after GC, private keys can be recovered from heap. **Remediation**: In pkg/identity/keys/keystore.go Load() method after line 175 (after key copy), add `for i := range plaintextKey { plaintextKey[i] = 0 }` before function return. Apply same pattern to all functions handling Ed25519/Curve25519 private keys: GenerateKeypair(), ExportKey(), ImportKey(). Validation: Write a test that allocates then zeros a key, triggers GC, and scans heap for key bytes (tricky but doable with unsafe and runtime.KeepAlive).

### HIGH

- [ ] **No simulation tests for gossip propagation** — test/ directory — Per ROADMAP.md milestone v1.0, simulation tests with 10–100 in-process libp2p nodes are required to verify gossip propagation, Shroud anonymity, and Resonance convergence. Currently, all tests are unit tests or single-host integration tests. No multi-node gossip latency validation exists. The 3-hop <500ms target is unverified. **Remediation**: Create `test/simulation/gossip_test.go` with `//go:build simulation` tag. Use in-memory libp2p transports to create 50 nodes in a mesh. Publish a Wave from node 0, measure time until received by 99% of nodes. Assert <3 seconds (allowing 6x the 500ms 3-hop target for multi-hop fanout). Use `go test -tags=simulation -v ./test/simulation`. Add CI job that runs simulation tests nightly. Validation: Run test, check output shows propagation times per node.
...
- [ ] **No rate limiting per peer** — pkg/networking/mesh/manager.go — Per SECURITY_PRIVACY.md §4 and ROADMAP.md milestone v1.0, "Rate limiting per peer — per-peer message rate caps" is required. Currently, any peer can flood the local node with GossipSub messages. Peer scoring penalizes bad behavior post-hoc but doesn't prevent the flood. A malicious peer can send 1000 Waves/second until scored out. **Remediation**: Add rate limiter to pkg/networking/gossip/pubsub.go Subscribe() message handler. Use `golang.org/x/time/rate` to create per-peer limiters (map[peer.ID]*rate.Limiter) with limit 10 msgs/sec, burst 20. In message handler before validation, check `if !limiter.Allow() { return }`. Store limiters in PubSub struct with periodic cleanup of stale entries (peer disconnected >5 min). Validation: Write test that simulates peer sending 100 msgs/sec, verify only ~10/sec processed.

- [ ] **No bootstrap peer fallback** — pkg/networking/discovery/bootstrap.go:50–80 — If all bootstrap peers in the hardcoded list are unreachable (offline or firewalled), the node runs in "isolated mode" (warning printed) with no peers. There's no fallback to alternative discovery (DNS, HTTP resolver). Per ROADMAP.md milestone v0.2, HTTP/DNS/IPFS gateway resolvers exist but aren't integrated into bootstrap flow. **Remediation**: In pkg/networking/discovery/bootstrap.go Bootstrap() function after line 70 (after DHT bootstrap attempt), if peer count is 0, try fallback resolvers in sequence: (1) DNSResolver for `_murmur._tcp.murmur.network` SRV records, (2) HTTPResolver for `https://bootstrap.murmur.network/peers.json`, (3) IPFSGatewayResolver for IPNS `/ipns/bootstrap.murmur.network`. Each resolver already exists in pkg/networking/discovery/. Add `fallbackResolvers []Resolver` field to BootstrapConfig. Validation: Start node with invalid bootstrap peers, verify it discovers peers via fallback.
...
4. **Security gaps in hot paths** (deduplication, rate limiting, key zeroing)
...
**Date**: 2026-05-04
**Validation**: `go test -race ./...` (54 packages pass), `go vet ./...` (clean), `go build ./cmd/murmur` (successful), binary startup test (CLI mode operational)
