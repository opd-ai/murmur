...
- *After 1 run*: Gist contains 4 ephemeral addresses (expired) and any IPFS-DHT peers seen during the run. Pages has a valid `peers.json`. IPNS record is live.
...
1. **App starts.** The `ResolverChain` in `pkg/networking/discovery/` is initialized with resolvers in priority order.
2. **Resolver 1 — Gist fetch (target: <1s).** HTTP GET to `https://gist.githubusercontent.com/opd-ai/murmur/raw/peers.json`. Response is a JSON array of signed multiaddrs. Signature is verified against the embedded `BOOTSTRAP_VERIFY_KEY` (public key, not secret). On success, attempt connection to up to 3 random entries. If ≥1 connects → **bootstrap complete**, remaining resolvers skipped.
3. **Resolver 2 — GitHub Pages fetch (target: <2s, CDN-cached).** HTTP GET to `https://opd-ai.github.io/murmur/peers.json`. Same signed format. On success, attempt connection. If ≥1 connects → **bootstrap complete**.
4. **Resolver 3 — IPFS DHT namespace query (target: <10s).** Dials the public IPFS bootstrap nodes (`/dnsaddr/bootstrap.libp2p.io/p2p/QmNnooDu7bfjPFoTZYxMNLWUQJyrVwtbZg5gBMjTezGAJN` et al., which are stable and permissioned for this use). Calls `routingDiscovery.FindPeers(ctx, "/murmur/bootstrap/v1")`. On success, attempt connection. If ≥1 connects → **bootstrap complete**.
5. **Resolver 4 — IPNS/CID fallback (target: <30s).** Reads `ipns.txt` from the Pages URL to get the latest CID, then fetches via `https://dweb.link/ipfs/<CID>/peers.json` (Cloudflare IPFS gateway, no auth). Same signed format. If ≥1 connects → **bootstrap complete**.
6. **Resolver 5 — Config file / `--bootstrap` flag.** User-supplied multiaddrs from `~/.murmur/config.toml` or the `--bootstrap` CLI flag. Always checked last; if provided, tried regardless of prior success.
7. **Failure path.** If all resolvers yield zero connections after their timeouts, the app starts in isolated mode with a UI warning and prompts the user to enter a peer address manually or retry after 60 seconds.
...
Job 2 — aggregate (needs: matrix probe, runs-on: ubuntu-latest):
  ...
  - Commit peers.json + ipns.txt to gh-pages branch
...
- `GITHUB_TOKEN` — Auto-injected by Actions; used only for Gist PATCH and gh-pages push.
- `BOOTSTRAP_GIST_ID` — Repository variable (not secret), the Gist ID. Set once during initial provisioning.
...
| IPFS-addressed copy | web3.storage / IPFS | Cloudflare/IPFS (free) | CI aggregate job via OIDC action | Go `IPFSGatewayResolver` (HTTP GET to dweb.link) |
...
- **`pkg/networking/discovery/`** — Add the following new files (existing files unchanged):
  ...
  - `ipfs_gateway_resolver.go` — `IPFSGatewayResolver`: fetches `ipns.txt` from Pages to get CID, then fetches `https://dweb.link/ipfs/<CID>/peers.json`. 20s timeout.
...
4. **IPFS Gateway (`dweb.link`)** — slow (20–30s: Pages fetch + gateway fetch), survives GitHub outage. *CI-maintained (content addressed, immutable once pinned).*
...
| web3.storage free tier discontinued | The IPFS layer is a fallback, not primary. Remove `IPFSGatewayResolver` from chain; Layers 1–3 remain intact. The `ipns.txt` file in Pages can be left as an empty string, which the resolver detects and skips. |
...
<!-- Existing PLAN.md content begins below this line -->
