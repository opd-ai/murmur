...
- **Gossip timestamp validator: TTL-aware past-drift window** (`pkg/networking/gossip/handlers.go`): `validateTimestamp` now accepts a `ttlSeconds int64` parameter; Waves are accepted if `(now − created_at) < wave.TTLSeconds`, allowing long-lived Waves to reach late-joining peers. Future timestamps are still rejected beyond MaxTimestampDrift (300 s). `Envelope.TTLSeconds` is populated from the Wave payload in `extractWaveFields`.
- **Health/metrics endpoints default to loopback** (`pkg/networking/health/health.go`, `pkg/config/config.go`, `pkg/app/murmur.go`): `Server.Start` now takes a `bindAll bool` parameter. When `false` (default), the HTTP server binds to `127.0.0.1` instead of `0.0.0.0`, preventing external enumeration of PeerID and Prometheus metrics. Bootstrap operators can set `Config.HealthBindAll = true` to restore public binding.
- **Unbiased Shroud relay selection** (`pkg/anonymous/shroud/circuit.go`): replaced modulo-biased `pickRandomUnusedIndex` with an unbiased Fisher-Yates shuffle using `crypto/rand.Int` + `math/big`. Eliminates statistical correlation between relay selections that could aid traffic-analysis adversaries.
- **ZeroBytes: dead-store-elimination resistant** (`pkg/identity/keys/keypair.go`): added `//go:noinline` directive and `runtime.KeepAlive(b)` after the zeroing loop to guard against hypothetical future compiler dead-store elimination of key material writes.
- **HKDF domain-separator salts** (`pkg/content/waves/veiled.go`, `pkg/anonymous/shroud/whisper.go`, `pkg/identity/recovery/recovery.go`): replaced `nil` salt in all three HKDF-SHA-256 invocations with fixed domain-separator byte strings (`"murmur-veil-salt-v1"`, `"murmur-whisper-salt-v1"`, `"murmur-recovery-salt-v1"`), improving auditability per RFC 5869 recommendations.
- **Legacy keystore `.bak` user warning** (`pkg/identity/keys/keystore.go`): `renameLegacyFile` now prints a visible security warning to stdout after renaming the legacy file, informing the operator to delete the backup once new keystores are verified.
- **AbyssalStore in-memory nonce risk documented** (`pkg/content/waves/abyssal.go`): added a prominent security note and `TODO` comment on `AbyssalStore` explaining that nonces are in plaintext process memory and should be migrated to encrypted bbolt storage.
- **Wave content filter DoS protection** (`pkg/content/filtering/filter.go`): `MuteKeyword` now returns `error` and enforces a `MaxKeywordPatterns = 100` limit, preventing pathological O(k×n) filter latency from unbounded keyword lists. Added `ErrTooManyPatterns` sentinel and `TestFilterMuteKeywordLimit` test.
...
### Added (2026-05-08 — ROADMAP Priority 2 validation)
- **Store-backed Join Game integration tests** (`pkg/pulsemap/game_actions_test.go`): `TestCountNearbyMechanics_WithStore_ReturnsCount` verifies that `countNearbyMechanics` returns ≥ 1 when an active `CipherPuzzle` is present in the store; `TestHandleJoinGame_WithStore_ShowsMechanicCount` confirms `handleJoinGame` produces a non-error toast containing the mechanic count; `TestHandleJoinGame_NoMechanics_ShowsUnavailableToast` verifies the empty-store path. Fulfills ROADMAP.md Priority 2 validation requirement.

### Fixed (2026-05-08 — AUDIT remediation: security-sensitive implementation gaps)
- **Veiled Wave key wrapping** (`pkg/content/waves/veiled.go`): replaced XOR-based symmetric key wrapping with X25519 Diffie-Hellman key exchange + HKDF-SHA-256, producing 32-byte wrapping keys. `wrapSymmetricKey` and `UnwrapSymmetricKey` now match the X25519 DH + HKDF scheme documented in SECURITY_PRIVACY.md. Round-trip and malformed-key tests confirm correctness.
- **Resonance claim contract** (`pkg/anonymous/resonance/claims.go`, `score_test.go`): deprecated `ClaimGenerator`/`ClaimVerifier` (which used an XOR-based arithmetic approximation) with explicit `// Deprecated:` godoc comments pointing to the canonical Ristretto-backed path. All tests for the claim layer now use `NewZKClaim`/`NewRistrettoClaimVerifier` from `pedersen.go`, ensuring only one cryptographically-correct claim path is exercised. Added `TestZKClaim*` suite covering generate/verify round-trip, threshold-not-met, expiry, replay, future timestamp, corrupted proof, and milestone thresholds.
- **Mobile share-sheet stub** (`pkg/identity/share_mobile.go`): created `//go:build android || ios` platform file implementing `openSystemShareImpl`. Registers a `MobileShareHandler` callback via `RegisterMobileShareHandler` for native share-sheet integration by the gomobile host, with a temp-file fallback when no callback is registered. Removed "not yet implemented" error from `OpenSystemShare`.
- **Onboarding completion invite generation** (`pkg/onboarding/screens/completion_screen.go`, `completion_screen_stub.go`): replaced the `MURMUR-XXXX-YYYY` hex-prefix placeholder with a call to `identity.GenerateInvitation` + `EncodeURI`, producing a real `murmur://` URI. Added `SetPeerID` method for the host application to inject the libp2p peer ID; falls back to deriving peer ID from the Ed25519 public key via `libp2pcrypto.UnmarshalEd25519PublicKey` + `peer.IDFromPublicKey`. Stub updated identically.
- **Invitation bootstrap fallback** (`pkg/onboarding/bootstrap/invitation.go`): added `IsBareP2PAddr(addr string) bool` helper so callers can detect when only a bare `/p2p/<peerID>` (no transport) was returned and must fall back to DHT discovery. Documented the bare-address contract in all three bootstrap functions. Added `TestIsBareP2PAddr`, `TestBuildBootstrapAddrFallbackIsBare`, and `TestBuildBootstrapAddrSignedIsDialable` tests.
- **Datacenter classification** (`pkg/networking/mesh/diversity.go`): implemented `IPClassification.IsDatacenter` with a curated list of 80+ CIDR ranges for major cloud/hosting providers (Cloudflare, AWS, GCP, Azure, DigitalOcean, Hetzner, Linode, OVH). Wired classification into `deriveRegionFromIP`—datacenter IPs now map to `RegionDatacenter` so the `RegionDiversityManager` can apply per-region peer limits and deprioritise centralised infrastructure. Updated test to validate known datacenter/non-datacenter fixtures and nil safety.
...
- **WASM architecture assessment — deferred full app integration (2026-05-07)**: Investigated full desktop/WASM UI parity. Found architectural mismatches: bbolt requires Unix syscalls unavailable in WASM, libp2p requires TCP/UDP transports only available on native OS. WASM support requires custom storage adapter (localStorage/IndexedDB), browser-native networking (WebRTC relay), and separate app architecture. Decision: defer WASM full integration to future phase. Current state: ✅ Boot.js initializes cleanly with event-driven callback, ✅ displays "WASM build: work in progress" status, ✅ desktop and WASM builds remain independent and non-breaking. See `AUDIT.md` 2026-05-07 WASM Architecture Assessment section for detailed analysis.

### Added
- **Bubble Tea TUI binary scaffold (2026-05-08)**: Added `cmd/murmur-tui` and new `pkg/tui/{views,components,styles,input,bridge}` architecture with tabbed Pulse Map/Identity/Waves/Anonymous/Onboarding/Networking terminal views, key/mouse handling, and baseline tests for model updates and deterministic snapshot flows.
- **TUI parity expansion batch (2026-05-08)**: Added layout-backed Pulse Map updates (search, node detail, minimap summary, bookmarks, edge list), onboarding bootstrap progress integration, identity recovery/cooldown/fingerprint/invite displays, thread-aware Waves compose/reply flow, expanded anonymous Shroud/echo index surfaces, and settings/layer-blend controls with UI-action emission hooks.
- **TUI parity closure batch (2026-05-08)**: Closed remaining P1 parity gaps including Pulse Map double-tap/zoom and color state tags, identity declaration editing and specter/masked sigils, Wave amplification + first-wave hints, anonymous relay/whisper + mechanics boards, onboarding skip/resume/hints/invitation/recovery/completion/returning surfaces, networking relay diagnostics + peer feed, settings config/overlay toggles, and cross-layer heatmap/marks/gifts/echo annotations.
- **TUI parity completion batch (2026-05-08)**: Closed all remaining P2 parity gaps with terminal equivalents for Pulse Map effects/background/contrast mode, Sparks and Pulse Beats surfaces, networking protocol reference visibility, theme switching, and wordlist source/regeneration controls; feature matrix now has no remaining `todo`/`deferred` rows.
- **UI audit prompt focused on first-time usability (2026-05-07)**: Added `UI_AUDIT.md`, a MURMUR-specific Ebitengine audit prompt that prioritizes intuitive use of the Pulse Map, onboarding clarity, obvious interaction affordances, and first-run comprehension over generic engine-only defect scanning.
...
- **Dynamic bootstrap server with live DHT peer distribution (2026-05-07)**: Reworked `cmd/bootstrap` from a static `-peers-file` server into a full libp2p bootstrap participant with persistent identity (`-state-dir`), DHT server-mode participation, peer exchange learning, and automatic recent-peer distribution in signed `/peers.json` responses.
- **Bootstrap container deployment assets (2026-05-07)**: Updated `Dockerfile.bootstrap` and `docker-compose.bootstrap.example.yml` for dynamic bootstrap mode with persistent state, exposed libp2p TCP/UDP ports, optional ngrok domain (`NGROK_DOMAIN`), and optional advertised public multiaddrs (`ANNOUNCE_ADDRS`).

### Changed
- **Build target now includes TUI binary (2026-05-08)**: Updated `Makefile` `build` target to produce both `bin/murmur` and `bin/murmur-tui` in one invocation; added TUI docs (`cmd/murmur-tui/README.md`, `docs/TUI_ARCHITECTURE.md`) and README status entry.
- **Release workflow action migration (2026-05-08)**: Updated `.github/workflows/build.yml` to replace `softprops/action-gh-release@v3` with `ncipollo/release-action@v1` in the `Create release` step, mapping inputs to the new action (`artifacts`, `generateReleaseNotes`) while preserving draft release behavior and release naming.
- **Transition/input remediation completion pass (2026-05-07)**: Closed remaining audit findings with targeted fixes in active Ebitengine paths. `pkg/pulsemap/game.go` now resets node-selection de-dup state on detail close; `pkg/ui/search.go` now hard-clears opacity on hide/toggle-off to prevent stale overlay ghosts; `pkg/onboarding/screens/identity.go` now blocks Enter-driven phase progression while display-name input is focused; `pkg/onboarding/screens/mode_screen.go` now uses responsive 4-column/2-column mode-card layout shared by draw/click/hover hit-testing; `pkg/pulsemap/interaction/touch.go` reset now clears deferred tap debounce state; `pkg/onboarding/screens/recovery_screen.go` now clears background each frame and uses scalable shared text face sizing from `pkg/onboarding/screens/helpers.go`; and `pkg/pulsemap/rendering/renderer.go` gates cross-layer artifact store lookups to Micro zoom to reduce transition stutter risk. Verified with `go test ./pkg/pulsemap/... ./pkg/ui ./pkg/onboarding/screens ./pkg/app`.
- **Audit remediation completion — transitions + input + responsive recovery (2026-05-07)**: Implemented deterministic onboarding phase sequencing in `pkg/app/ui.go` (identity -> mode -> bootstrap before Pulse Map handoff), added overlay input isolation for visible SearchBar (`pkg/ui/search.go`), removed per-frame minimap node-slice churn via reusable scratch buffer (`pkg/pulsemap/game.go`), decoupled compose-button hit-testing from Draw-timing (`pkg/ui/compose.go`), and made RecoveryScreen layout/hit-testing viewport-responsive (`pkg/onboarding/screens/recovery_screen.go`). Added/updated focused tests including search input-consumption behavior.
- **UI clarity remediation batch (2026-05-07)**: Implemented and validated audit-driven usability fixes across active desktop paths. `pkg/ui/device_management.go` now has full pointer interaction wiring for add/revoke/confirm flows; `pkg/ui/settings.go` now renders row labels, descriptions, and live control values; `pkg/pulsemap/game.go` now initializes/draws minimap and uses non-dropping wave-result enqueue behavior; onboarding screens (`pkg/onboarding/screens/returning_screen.go`, `identity.go`, `mode_screen.go`, `bootstrap_screen.go`, `completion_screen.go`) now support explicit Enter/Space/Escape progression with no forced auto-continue; `pkg/ui/node_detail.go` now renders explicit no-data/connection-state feedback; `pkg/ui/radial_menu.go` now renders configured glyph icons instead of placeholder dots.
- **UI remediation verification sweep (2026-05-07)**: Passed targeted validation on modified UX surfaces with `go test ./pkg/ui ./pkg/pulsemap/... ./pkg/onboarding/screens` and static checks via `go vet ./pkg/ui ./pkg/pulsemap/... ./pkg/onboarding/screens`.
- **UI transition/input hardening follow-up (2026-05-07)**: Fixed returning-user screen handoff race by running the welcome screen synchronously, switched Pulse Map context-menu target resolution to pointer hit-testing (with selection fallback), kept world tick/render updates active while modal panels consume input, made first-wave onboarding backspace UTF-8/rune-safe, changed Search cursor blink to tick-based timing, and made camera momentum/animation interpolation time-based for smoother behavior under variable frame rates.
- **Ebitengine transition/input audit remediation (2026-05-07)**: Resolved critical/high UI reliability issues across onboarding and Pulse Map input routing. Fixes include one-shot returning-screen continuation with clean termination, safe returning-screen app handoff without premature app cancellation, one-shot keypair/specter generation triggered from Update (not Draw), global-shortcut blocking while modal UI is active, `Ctrl+N` conflict resolution (compose toggle no longer also triggers network recenter), rune-safe backspace handling for onboarding/settings text input, write-lock-safe geometry caching in compose/passphrase panels, and minimap static-layer caching to avoid unnecessary per-frame redraw work.
- **First-run onboarding handoff repair (2026-05-07)**: Fixed the blocking desktop first-run path in `pkg/app/ui.go`. Onboarding no longer logs a Pulse Map transition and then cancels the application; it now persists the first-run completion flag and performs a normal in-session transition from onboarding UI to Pulse Map UI. Validated with focused app/race tests and full `go build ./...`.
- **Bootstrap Docker DNS fallback (2026-05-07)**: Updated `Dockerfile.bootstrap` and `docker-compose.bootstrap.example.yml` so bootstrap image builds can bypass broken Docker DNS resolution. The compose example now builds with host networking, and the Dockerfile accepts `GOPROXY`, `GOSUMDB`, and `GOFLAGS` build args for restricted network environments. Added operator guidance to `docs/BOOTSTRAP_OPERATION.md`.
- **Transport integration test repair (2026-05-07)**: Rebuilt `pkg/networking/transport/integration_test.go` after file corruption. Restored valid Go syntax, aligned diagnostics calls to current signatures (`CheckTor/CheckI2P/CheckAll` with `context.Context`), added deterministic host config helper with generated Ed25519 keys for `NewHost`, and fixed garlic/onion multiaddr parsing assertions.
...
### Changed
- CI build matrix now produces native per-OS desktop artifacts only (Linux/macOS/Windows) instead of cross-compiled artifacts.
- Native build job now compiles `./cmd/murmur` with Ebitengine GUI enabled (`CGO_ENABLED=1`, no `noebiten` tag).
- Removed js/wasm and cross-target `GOOS/GOARCH` build logic from the CI artifact matrix to enforce native-only executable publishing.
